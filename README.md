...
数位板原始坐标轴与横屏方向一致，因此本方案默认使用 **libinput 恒等校准矩阵**
（不旋转），并让 KDE/libinput 把数位板完整铺满屏幕：

```text
1 0 0 0 1 0
```

内核驱动本身只负责正确解析该设备的 HID 报告并上报标准输入事件；方向与
铺满范围由 `udev` 规则和 KDE 数位板默认设置共同保证。
...
5. 写入 libinput 恒等校准规则 `/etc/udev/rules.d/99-fans-joy-s2-calibration.rules`
6. 设置 KDE 数位板默认值 `Orientation=0`、全屏映射、`MapToWorkspace=false`
7. 立即加载模块

如果以后想额外旋转，可以显式传入 libinput 矩阵，例如 90° 逆时针：
...
描述符钳制坐标，导致屏幕只能覆盖一半。最终实测确认该数位板的原始横屏坐标
轴已经和横屏一致，因此标准且稳定的做法是：

```text
内核驱动上报原始坐标 + libinput 恒等矩阵 + KDE Orientation=0 / 全屏映射
```
...
the tablet coordinate system on Ubuntu or Kubuntu when a different physical
